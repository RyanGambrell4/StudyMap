...
function buildTopics(course, coachPlanForCourse, sessionsForCourse, struggleTopicSet) {
  ...
  const weeklyFocus = coachPlanForCourse?.plan?.weeklyFocus
  if (Array.isArray(weeklyFocus)) {
    for (const wk of weeklyFocus) {
      for (const t of (wk?.keyTopics || [])) {
        ...
        seen.set(key, {
          ...
          provenance: 'coach_plan.weeklyFocus.keyTopics',
          source: 'derived',
        })
      }
    }
  ...
  const sessions = coachPlanForCourse?.plan?.sessions
  if (Array.isArray(sessions)) {
    for (const s of sessions) {
      ...
      seen.set(key, {
        ...
        lastTouchedAt: null,
        isStruggle: struggleTopicSet.has(key),
        provenance: 'coach_plan.plan.sessions[].topic',
        source: 'derived',
      })
    }
  }
  ...
  for (const s of sessionsForCourse) {
    ...
    if (seen.has(key)) {
      ...
      cur.sessionCount += 1
      if (!cur.lastTouchedAt || String(s.dateStr) > String(cur.lastTouchedAt)) {
        cur.lastTouchedAt = s.dateStr
      }
    } else {
      seen.set(key, {
        name: t,
        sessionCount: 1,
        lastTouchedAt: s.dateStr,
        isStruggle: struggleTopicSet.has(key),
        provenance: 'completed_sessions.sessionType',
...
function buildSessions(sessionsForCourse, orphanCount) {
...
function buildPlanSection(coachPlanForCourse) {
  ...
  return {
    ...
    struggles: Array.isArray(coachPlanForCourse.struggles) ? coachPlanForCourse.struggles : [],
...
async function assemble(userId, courseId, sections, topic, budgetChars) {
  ...
  const sessionsSection = wanted.has('sessions')
    ? buildSessions(sessionsForCourse, orphanSessions)
    : null
...
function renderDeadlines(d) {
  ...
  for (const it of d.items) {
    ...
    lines.push(`- ${it.date} — ${it.title} (${tags})`)
  }
  return lines.join('\n')
}
...
function renderSessions(s, remainingChars) {
  if (!s) return ''
  if (!s.recent?.length) return 'RECENT SESSIONS: (none)'
  ...
  return kept.length ? [header, ...kept, `- ...(+${rows.length - kept.length} sessions trimmed for budget)`].join('\n') : ''
}
