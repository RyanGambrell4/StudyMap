...
/**
 * Find a course by the id a caller supplied, accepting BOTH shapes the product
 * actually stores.
 *
 ...
 * a number. Verified in production on 2026-08-28: of 861 sessions across all 35
 * users who have any, **861 carry a number and none carry the opaque id.**
 *
 * So a strict `String(c.id) === String(courseId)` match could never succeed for
 * a session-launched request, and every AI tool opened from inside a focus
 * session failed with "course N not found" — silently, for every user, for as
 * long as both shapes have coexisted. It surfaced because the one paying
 * customer hit it 31 times in thirteen minutes.
 *
 * The real repair is to make the scheduler write true ids, but that leaves
 * every existing session broken and needs a migration. Accepting the index here
 * fixes all historical data at once, needs no migration and no client deploy.
 *
 * Order matters: the opaque id is tried FIRST, so a course whose id happens to
 * be the string "3" still wins over index 3. The index is only ever a fallback
 * for a value that matched nothing, and only when it is a real in-bounds
 * integer — never for a float, a negative, or a non-numeric string.
 */
export function resolveCourseIndex(courses, courseId) {
  if (!Array.isArray(courses) || courses.length === 0) return -1
  if (courseId === null || courseId === undefined || courseId === '') return -1

  const byId = courses.findIndex(c => c && c.id !== undefined && String(c.id) === String(courseId))
  if (byId !== -1) return byId

  // Legacy session shape: courseId is the array index.
  const asIndex = Number(courseId)
  if (Number.isInteger(asIndex) && asIndex >= 0 && asIndex < courses.length) {
    return asIndex
  }
  return -1
}
...
async function loadUserAndCourse(userId, courseId) {
  ...
  const courseIndex = resolveCourseIndex(courses, courseId)
  if (courseIndex === -1) {
