...
describe('plan is read from user_billing, not the user-writable blob', () => {
  ...
  it('tolerates the table not existing yet, but only that', () => {
    // Deploy ordering must not matter: this code ships before the migration
    // runs. Conflating "table not created yet" with "the read failed" would
    // fail closed on every AI endpoint the moment it deployed, and skipping
    // the write on a missing table without falling back to the mirror would
    // stop the counter incrementing and hand out unlimited AI. Both are
    // one-line mistakes, so both are pinned.
    const src = stripComments(read('lib/server/billing.js'))
    expect(src).toContain('isMissingRelation')
    // Read: missing table falls back to the legacy blob rather than erroring.
    expect(src).toMatch(/if \(isMissingRelation\(error\)\)[\s\S]{0,400}readLegacyAsBilling/)
    // Writes: missing table skips the billing write and still mirrors.
    const writeGuards = src.match(/if \(error && !isMissingRelation\(error\)\)/g) ?? []
    expect(writeGuards.length).toBeGreaterThanOrEqual(3)
  })
