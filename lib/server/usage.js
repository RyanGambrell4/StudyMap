 ...
 */
export async function reserveAiUsage(req, { verified } = {}) {
  ...
  log('ai.gate.reserved', { userId, plan, used, limit: limit === Infinity ? null : limit })
  ...
  const gate = {
    ...
    async commit() {
      ...
      committed = true
      return commitReservation({ supabase, userId, sub, plan, used, newMonth })
    },
...
async function commitReservation({ supabase, userId, sub, plan, used, newMonth }) {
  const updatedSub = {
    ...sub,
    plan: sub.plan ?? plan,
  ...
  log('ai.gate.commit', { userId, plan, used: used + 1 })
  return { ok: true }
}
