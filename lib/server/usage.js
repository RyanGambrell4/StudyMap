...
import { readBilling, commitUsage } from './billing.js'
import { checkGlobalSpendCap } from './aiCost.js'
...
export async function reserveAiUsage(req, { verified, cost = 1 } = {}) {
  ...
  // Email confirmation is deliberately NOT checked here. It used to be, which
  // meant an unconfirmed account could sign up, get into the product, and then
  // have every AI action fail with a 403 it could do nothing about. The policy
  // is now: confirmation gates money, not use. requireConfirmedEmail() in
  // api/stripe.js is the one place that still enforces it.

  const supabase = getAdminClient()

  // Read from public.user_billing, not user_data.subscription.
  //
  // This is the whole point of the change. user_data.subscription is writable
  // by the user it belongs to, and the browser writes it wholesale, so reading
  // the plan and the counter from there meant every quota in the product could
  // be raised by the person it was limiting. user_billing has no INSERT,
  // UPDATE or DELETE policy; only the service role can write it.
  const read = await readBilling(supabase, userId)
  if (!read.ok) {
    // Fail closed. A read failure must not be indistinguishable from "this user
    // has plenty of quota left" — that is how a database blip turns into free
    // AI for everyone.
  ...
  const bonus = plan === 'free' ? (Number(billing.bonusAiActions) || 0) : 0
  ...
  if (!rlResult.allowed) {
    // 429 for a real rate limit, 503 when Redis is down and a free user is
    // refused for it. The difference matters to the client: one means slow
    // down, the other means try again shortly.
    log('ai.gate.ratelimit', { userId, error: rlResult.error, degraded: !!rlResult.degraded })
    return { ok: false, status: rlResult.status ?? 429, error: rlResult.error }
  }

  // Org-wide daily spend cap. Every other ceiling here is per user and so
  // scales with the number of accounts; this one does not. It fails open by
  // design — it is a backstop behind the Anthropic console spend limit, and a
  // fault in it must not be able to take AI down on its own.
  const spend = await checkGlobalSpendCap()
  if (!spend.ok) {
    log('ai.gate.spend_cap', { userId, plan, spentUsd: spend.spentUsd, capUsd: spend.capUsd })
    return {
      ok: false,
      status: 503,
      error: 'AI is briefly unavailable while we catch up on demand. Please try again later today.',
    }
  }

  const newMonth = isNewMonth(billing.aiQueriesResetAt)
  const used = newMonth ? 0 : (billing.aiQueriesUsed ?? 0)
  ...
  const gate = {
    ...
    async commit() {
      ...
      committed = true
      return commitReservation({ supabase, userId, billing, plan, used, newMonth, cost })
    },
...
async function commitReservation({ supabase, userId, billing, plan, used, newMonth, cost = 1 }) {
  // Writes user_billing first (the copy that decides anything) and mirrors into
  // user_data.subscription for the readers that have not moved yet. Note the
  // plan and status are NOT written here: this function may only ever move the
  // counters. Changing a plan is Stripe's job, and giving the usage path the
  // ability to write one is how it ends up writable from somewhere it should
  // not be.
  const written = await commitUsage(supabase, userId, {
    aiQueriesUsed: used + cost,
    aiQueriesResetAt: newMonth
      ? new Date().toISOString()
      : (billing.aiQueriesResetAt ?? new Date().toISOString()),
    ...
    firstGenerationAt: billing.firstGenerationAt ?? new Date().toISOString(),
  })
  ...
  if (!written.ok) {
    ...
    console.error('[usage] commit write error', written.error)
    log('ai.gate.commit_failed', { userId, plan, error: written.error })
 ...
 */
export async function verifyAuth(req, { requireEmailConfirmed = false } = {}) {
