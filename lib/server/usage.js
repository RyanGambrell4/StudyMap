...
import { applyServerUsagePatch } from './subscriptionMerge.js'

export { SERVER_USAGE_KEYS, applyServerUsagePatch } from './subscriptionMerge.js'
...
export async function verifyAndCheckAiUsage(req) {
  ...
  // Re-read immediately before writing rather than reusing the snapshot taken
  // at the top of this function.
  //
  // `subscription` is a single JSON column with two writers: this one (service
  // role) and the browser (src/lib/subscription.js, which records per-feature
  // usage in `feature_usage`). Both used to write the WHOLE column from a
  // snapshot, so whichever wrote last silently erased the other's keys. The
  // observable result was that `feature_usage` did not exist on a single one of
  // 777 user rows, which meant every non-AI free limit (practice exams,
  // blueprints, quiz bursts, brain dumps, exam rescue, coach plans) was
  // unenforced AND unmeasured for the product's entire history.
  //
  // Re-reading here collapses the clobber window to a few milliseconds and, more
  // importantly, means this writer only ever touches the keys it owns.
  ...
  if (refetchErr) {
    console.error('[usage] refetch before write failed', refetchErr)
    // Fall back to the earlier snapshot rather than failing the user's request.
  }

  const latest = (refetchErr ? null : freshRow?.subscription) ?? sub ?? {}

  const updatedSub = applyServerUsagePatch(latest, {
    plan: latest.plan ?? sub.plan ?? plan,
    status: latest.status ?? sub.status ?? 'active',
    aiQueriesUsed: used + 1,
    aiQueriesResetAt: newMonth
      ? new Date().toISOString()
      : (latest.aiQueriesResetAt ?? sub.aiQueriesResetAt ?? new Date().toISOString()),
