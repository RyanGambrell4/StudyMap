...
export async function sendFounderCancellationEmail(toEmail, opts = {}) {
  ...
  const text = [
    `Hey ${firstName},`,
    '',
    `We're the team behind StudyEdge AI.`,
    '',
    `We saw you cancelled your Pro trial. No hard feelings at all — we just want to`,
    `make the product better, and we read every reply.`,
    '',
    `Can you spare 60 seconds and hit reply with the one thing that made you`,
    `stop? Even one word helps. A few examples of the kind of thing we're after:`,
    '',
    ...
    `  - "Missing feature X"`,
    '',
    `Whatever it is, we want to know. If it's a bug, we'll fix it this week.`,
    '',
  ...
  // Reply-to defaults to the same StudyEdge address as `from` so replies
  // stay inside the studyedge.com domain and never leak into a personal
  // inbox. Overridable via FOUNDER_REPLY_EMAIL for whichever inbox actually
  // gets read (e.g. ryan@getstudyedge.com once that mailbox is set up).
  const replyTo = process.env.FOUNDER_REPLY_EMAIL || 'ryan@mail.getstudyedge.com'

  try {
    await resend.emails.send({
      from: 'StudyEdge AI Team <ryan@mail.getstudyedge.com>',
      replyTo,
      to: toEmail,
