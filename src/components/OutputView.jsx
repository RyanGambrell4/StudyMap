    ...
    <>
      ...
      >
        ...
        {activeSection === 'coach' && (
          <StudyCoachView
            ...
            onRescheduleSession={(sessionId, newDateStr) => { handleSessionMove(sessionId, newDateStr, null, null); track('session_rescheduled', { source: 'dashboard_missed_banner' }) }}
      ...
      <CommandPalette actions={(() => {
        ...
        const list = [
          ...
          { id: 't-timed',       group: 'Tools',    label: 'Timed Challenge',   hint: 'Race the clock on any topic',   run: () => { track('feature_opened', { feature: 'timed_challenge', source: 'cmdk' }); setShowTimedChallenge(true) }, keywords: 'timed challenge race clock speed' },
          { id: 't-podcast',     group: 'Tools',    label: 'Study Podcast',     hint: 'Listen and learn',              run: () => { track('feature_opened', { feature: 'podcast', source: 'cmdk' }); setShowPodcast(true) }, keywords: 'podcast audio listen study' },
        ]
        courses?.forEach((c, i) => {
          if (!c?.name) return
          list.push({
