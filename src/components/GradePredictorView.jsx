...
export default function GradePredictorView({ courses, onEditCourse, userId, onShowPaywall, initialCourseIdx = 0 }) {
  ...
  return (
    <div className="px-4 py-6 max-w-2xl mx-auto">
      {/* Header */}
      <div className="mb-5">
        <h1 className="text-2xl font-bold text-slate-900 mb-1">Grade Predictor</h1>
      ...
      {view === 'form' ? (
        <div className="bg-white border border-slate-200 rounded-2xl p-5">
          {/* Target Grade */}
          <div className="mb-5">
            <label className="block text-xs text-slate-500 uppercase tracking-widest font-bold mb-2">Target Grade</label>
          ...
          <div className="mb-4">
            <div className="flex items-center justify-between mb-2">
              <label className="text-xs text-slate-500 uppercase tracking-widest font-bold">Grade Components</label>
              ...
              {components.map((comp, i) => (
                <div key={i} className="grid grid-cols-[1fr_60px_90px_80px_28px] gap-2 items-center">
                  <input
                    ...
                    className="w-full bg-slate-50 border border-slate-200 rounded-lg px-2.5 py-2 text-sm text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-1 focus:ring-indigo-500/50"
                  />
                  <div className="relative">
                    <input
                      ...
                      className="w-full bg-slate-50 border border-slate-200 rounded-lg px-2 py-2 text-sm text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-1 focus:ring-indigo-500/50 pr-5"
                    />
                  ...
                  <select
                    ...
                    className="w-full bg-slate-50 border border-slate-200 rounded-lg px-2 py-2 text-sm text-slate-800 focus:outline-none focus:ring-1 focus:ring-indigo-500/50"
                  >
                  ...
                  <input
                    ...
                    className="w-full bg-slate-50 border border-slate-200 rounded-lg px-2 py-2 text-sm text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-1 focus:ring-indigo-500/50"
                  />
                  <button
                    onClick={() => removeComponent(i)}
                    disabled={components.length === 1}
                    className="w-7 h-7 flex items-center justify-center rounded-lg text-slate-400 hover:text-red-400 hover:bg-red-50 transition-colors disabled:opacity-30 disabled:pointer-events-none"
                  >
            ...
            <button
              ...
              className="mt-3 flex items-center gap-1.5 text-sm font-medium text-slate-500 hover:text-indigo-500 transition-colors"
            >
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-5">
        <div className="flex items-center gap-4 mb-5">
          ...
          <div>
            <p className="text-base font-bold text-slate-900 mb-1">{clean(course.name)}</p>
            <div
        ...
        <div className="grid grid-cols-2 gap-3 mb-5">
          <div className="bg-slate-50 rounded-xl px-3 py-3">
            ...
            <p className="text-lg font-bold text-slate-900">{prediction.predictedGrade?.toFixed(1)}%</p>
          </div>
          <div className="bg-slate-50 rounded-xl px-3 py-3">
            <p className="text-[10px] font-semibold text-slate-400 uppercase tracking-wide mb-0.5">Needed on Remaining</p>
            <p className={`text-lg font-bold ${(prediction.gradeNeededOnRemaining ?? 0) > 100 ? 'text-red-400' : 'text-slate-900'}`}>
        ...
        {prediction.keyFactors?.length > 0 && (
          <div className="mb-4">
            <p className="text-xs font-semibold text-slate-400 uppercase tracking-widest mb-2">Key Factors</p>
        ...
        {prediction.recommendations?.length > 0 && (
          <div className="mb-4">
            <p className="text-xs font-semibold text-slate-400 uppercase tracking-widest mb-2">Recommendations</p>
            <ul className="space-y-1.5">
        ...
        {prediction.weakAreas?.length > 0 && (
          <div className="bg-amber-50 border border-amber-200 rounded-xl px-3 py-2.5 mb-4">
            ...
            <p className="text-sm text-amber-700">{prediction.weakAreas.join(', ')}</p>
          </div>
        )}
