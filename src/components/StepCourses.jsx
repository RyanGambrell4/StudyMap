      ...
      {adding ? (
        <div className="bg-slate-800/50 rounded-2xl border border-slate-700/60 p-6 mb-5">
          ...
          <div className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-slate-400 mb-1.5">Exam Date <span className="text-slate-600 font-normal">(optional)</span></label>
              <input
                type="date"
                value={form.examDate}
                min={today}
            ...
            <div className={`rounded-xl border overflow-hidden transition-all ${syllabusOpen ? 'border-indigo-500/40 bg-indigo-500/5' : 'border-slate-700'}`}>
              <button
                type="button"
                onClick={() => { setSyllabusOpen(x => !x); if (form.syllabusFile && syllabusOpen) setForm({ ...form, syllabusFile: null }) }}
                className="w-full flex items-center gap-3 p-4 text-left"
              >
                <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${syllabusOpen ? 'bg-indigo-500/20 text-indigo-400' : 'bg-slate-700/60 text-slate-500'}`}>
              ...
              {syllabusOpen && (
                <div className="border-t border-slate-700/60 px-4 pb-4 pt-3 flex flex-col gap-2">
                  <button type="button" onClick={() => syllabusRef.current?.click()} className="flex items-center gap-2 px-4 py-2.5 rounded-lg bg-indigo-500/10 border border-indigo-500/30 text-slate-200 text-sm font-medium hover:bg-indigo-500/15 transition-colors">
                    <svg className="w-4 h-4 text-indigo-400 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4M17 8l-5-5-5 5M12 3v12"/></svg>
                    {form.syllabusFile ? form.syllabusFile.name : 'Choose file (PDF, DOCX, image)'}
                  </button>
                  <p className="text-xs text-slate-600">You can also import after adding the course from the Courses page.</p>
                </div>
              )}
            </div>
          ...
          <div className="flex gap-3 mt-5">
            ...
            {courses.length > 0 && (
              <button onClick={() => { setAdding(false); setErrors({}); setSyllabusOpen(false) }} className="px-5 bg-slate-700/70 hover:bg-slate-700 text-slate-300 font-medium py-3 rounded-xl transition-colors">
                Cancel
              </button>
            )}
          </div>
