export default function StudyNowCard({ nextSession, completedIds, onStartFocus }) {
  if (!nextSession) {
    return (
      <div className="no-print flex items-center gap-4 bg-emerald-50 border border-emerald-200 rounded-2xl px-6 py-5 mb-6">
        <div className="text-emerald-600"><svg className="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4M7.835 4.697a3.42 3.42 0 001.946-.806 3.42 3.42 0 014.438 0 3.42 3.42 0 001.946.806 3.42 3.42 0 013.138 3.138 3.42 3.42 0 00.806 1.946 3.42 3.42 0 010 4.438 3.42 3.42 0 00-.806 1.946 3.42 3.42 0 01-3.138 3.138 3.42 3.42 0 00-1.946.806 3.42 3.42 0 01-4.438 0 3.42 3.42 0 00-1.946-.806 3.42 3.42 0 01-3.138-3.138 3.42 3.42 0 00-.806-1.946 3.42 3.42 0 010-4.438 3.42 3.42 0 00.806-1.946 3.42 3.42 0 013.138-3.138z" /></svg></div>
        <div>
          <p className="font-bold text-emerald-700 text-base">All sessions complete!</p>
          <p className="text-slate-600 text-sm mt-0.5">You've finished every scheduled session. Incredible work.</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div
      className="no-print relative overflow-hidden bg-white rounded-2xl px-6 py-5 mb-6"
      style={{ border: '1px solid rgba(0,0,0,0.07)', boxShadow: '0 1px 3px rgba(0,0,0,0.06)' }}
    >
      <div
        className="absolute inset-0 pointer-events-none"
        style={{ background: `radial-gradient(ellipse 80% 100% at 0% 50%, ${nextSession.color.dot}14, transparent 70%)` }}
      />
      ...
      <div className="relative flex items-center gap-5 flex-wrap">
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <h3 className="text-slate-900 font-bold text-lg leading-tight">{nextSession.courseName}</h3>
            {isFinal && (
          ...
          <div className="flex items-center gap-2 mt-1.5 flex-wrap">
            <span className="text-sm text-slate-600">{nextSession.sessionType}</span>
            <span className="text-slate-300">·</span>
            {nextSession.startTime ? (
              <span className="text-sm text-slate-600">{nextSession.startTime} – {nextSession.endTime}</span>
            ) : (
              <span className="text-sm text-slate-600">{nextSession.duration} min</span>
            )}
            <span className="text-slate-300">·</span>
