...
function computeMission({ courses, upcomingExam, weakSpots, todaySessions, completedIds, streak, coachPlans }) {
  ...
  if (upcomingExam && upcomingExam.days <= 1) {
    return {
      ...
      context: 'Exam Rescue shows exactly what to study in the hours you have left.',
  ...
  if (upcomingExam && upcomingExam.days <= 3) {
    return {
      ...
      context: 'Open your Cheat Sheet to see the highest-priority topics, then do a Brain Dump on your weakest one.',
  ...
  if (todaySessions.length > 0 && completedIds) {
    ...
    if (uncompleted.length > 0) {
      ...
      return {
        ...
        context: `${clean(s.courseName ?? '')} ${s.duration ? `· ${s.duration} min` : ''}`.trim() || 'Your session is ready.',
        cta: 'Start Session',
        ctaKey: 'focus',
