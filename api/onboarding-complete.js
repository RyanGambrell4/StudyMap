...
export default async function handler(req, res) {
  ...
  const { firstName, yearLevel, learningStyle, preferredTime } = body
  ...
  // userId, which left `email` unvalidated on its way to `to:`. See
  ...
  if (authUserErr) {
    reportQueryError(authUserErr, { table: 'auth.users', context: 'onboarding-complete recipient lookup' })
      ...
      <tr><td style="background:#FFFFFF;border-radius:16px;border:1px solid rgba(0,0,0,0.07);padding:32px 32px 28px;">
        ...
        <table cellpadding="0" cellspacing="0" style="width:100%;margin-top:20px;">
          <tr><td style="background:#F4F7FF;border-radius:12px;border:1px solid rgba(59,97,196,0.15);padding:14px 18px;text-align:center;">
            ...
            <p style="margin:0 0 10px;font-size:13px;color:#6B6B6B;line-height:1.55;">${trialUsed ? 'Unlimited courses, AI tutoring, brain dumps, cheat sheets, and more. $9.99/mo.' : 'Card required · Cancel before day 8 and pay nothing.'}</p>
            <a href="https://getstudyedge.com/app?signup=1&plan=pro&billing=monthly&trial=1&utm_source=email&utm_medium=lifecycle&utm_campaign=onboarding_complete" style="display:inline-block;background:#E8531A;color:#FFFFFF;font-size:13px;font-weight:600;text-decoration:none;border-radius:8px;padding:10px 22px;">${trialUsed ? 'Upgrade to Pro →' : 'Start free 7-day trial →'}</a>
          </td></tr>
        </table>` : ''}
      </td></tr>
